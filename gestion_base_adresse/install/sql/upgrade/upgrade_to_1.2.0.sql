...
CREATE OR REPLACE VIEW adresse.v_export_bal AS
 SELECT ''::text AS uid_adresse,
   ...
   FROM (((((adresse.point_adresse p
     ...
     LEFT JOIN adresse.commune_deleguee cd ON ((cd.id_com_del = rc.id_com_deleguee) AND St_Contains(cd.geom, p.geom)))
...
UNION
 SELECT ''::text AS uid_adresse,
   ...
   FROM (((adresse.lieux_dits ld
     ...
     LEFT JOIN adresse.commune_deleguee cd ON ((cd.id_com_del = rc.id_com_deleguee) AND St_Contains(cd.geom, ld.geom)))
